...
<body>
  ...
  <?php 
      $id = $_GET['updateid'] ;
      $selectSQL = "SELECT * FROM `usager` WHERE Numero_usager=$id" ;
      $result=@mysql_query($selectSQL,$idcon);
      $row = mysql_fetch_assoc($result) ;
      $id_Orig = $row['Numero_usager'] ;
      $Nom_Orig = $row['Nom'] ;
      ...
      if(isset($_POST['Modifier'])) {
        ...
        $Statut = $_POST['statut'] ;
        $Email = $_POST['email'] ;
        $updatetSQL = "UPDATE `usager` SET  `Nom`='$Nom' ,`Prenom`='$Prenom' ,`Adresse`='$Adresse' ,`Statut`='$Statut' ,`Email`='$Email' WHERE Numero_usager=$id " ;
